...
// ========================= continue to play tones============================
		...
		// -----play first tone ------
		clearNum = randInt(1,11,10)
		clearNum = clearNum[0]

		console.log(tone1[trial-1])
		clearing_duration = 650
		context_duration = 300
		onset2 = onset[trial-1] + 1000 + clearing_duration + context_duration
		// ======== Pre exposure - Cleaning tone ============
		setTimeout(function () {clearing[clearNum].play()},500);
		setTimeout(function () {clearing[clearNum].pause()},500 + clearing_duration);
		// ======== Pre exposure - Context tone ============
		setTimeout(function () {s1[context_tone[trial-1]].play()},1000 + clearing_duration);
		setTimeout(function () {s1[context_tone[trial-1]].pause()},1000+ clearing_duration + context_duration);

		// ======== Test trial ============
		setTimeout(function () {$('.taskCue').show();},onset2);
		setTimeout(function () {s1[tone1[trial-1]].play()},onset2);
		// -----stop first tone ------
		setTimeout(function () {s1[tone1[trial-1]].pause()},onset2+duration[trial-1]+ISI[trial-1]-10);
		setTimeout(function () {s1[tone1[trial-1]].currentTime = 0},onset2+duration[trial-1]+ISI[trial-1]-10);
		// -----play second tone ------
		setTimeout(function () {s2[tone2[trial-1]].play()},onset2+duration[trial-1]+ISI[trial-1]);
		setTimeout(function () {$('.taskCue').hide();},onset2+duration[trial-1]+duration[trial-1]+ISI[trial-1]);
		callTimeout(onset2+duration[trial-1]+duration[trial-1]+ISI[trial-1],trial); 
	}
}
...
function getAcc(){
	if ((tone1[trial-1]<tone2[trial-1]) == response[trial-1]){
		acc[trial-1] = 1;
	}else{
		acc[trial-1] = 0;
	} 
